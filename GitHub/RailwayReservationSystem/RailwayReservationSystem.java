...
public class RailwayReservationSystem{
    ...
    static void initializeTrains() {
        ...
        trains.add(new Train("Delhi - Jaipur", "7:00", 100, 2013));
    ...
    static void displayTrainInformation() {
        ...
        for(int i=0;i<trains.size();i++){
            System.out.println(trains.get(i).trainNumber + "\t\t" + trains.get(i).name + "\t\t" + trains.get(i).time + "\t" + trains.get(i).passengerStrength);
        }
    }
    ...
    static void checkSeatAvailability(Scanner scanner) {
        ...
        for(int i=0;i<trains.size();i++){
            if(trains.get(i).trainNumber==trainNumber){
                int availableSeats=trains.get(i).passengerStrength-countBookedSeats(trainNumber);
                System.out.println("Available seat on Train "+trainNumber+": "+availableSeats);
                return;
            }
        }
    ...
    static int countBookedSeats(int trainNumber) {
        ...
        for(int i=0;i<tickets.size();i++){
            if(tickets.get(i).trainNumber==trainNumber){
                count++;
            }
        }
        return count;
